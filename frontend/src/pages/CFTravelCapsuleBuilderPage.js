...
// Custom Feature: Travel capsule builder
// Travel capsule builder: Trip details input (duration, activities, weather); AI generates minimal packing list from existing wardrobe with predicted cost-per-day
import React, { useState } from 'react';

export default function CFTravelCapsuleBuilderPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-travel-capsule-builder/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Travel capsule builder</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Travel capsule builder: Trip details input (duration, activities, weather); AI generates minimal packing list from existing wardrobe with predicted cost-per-day</p>
