...
// Gap (Non-AI): Limited travel features (only packing lists)
// Limited travel features (only packing lists)
import React, { useState } from 'react';

export default function GapLimitedTravelFeaturesOnlyPackingListsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-travel-features-only-packing-lists/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Limited travel features (only packing lists)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Limited travel features (only packing lists)</p>
