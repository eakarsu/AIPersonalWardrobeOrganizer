...
// Gap (Non-AI): No mobile app
// No mobile app
import React, { useState } from 'react';

export default function GapNoMobileAppPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-mobile-app/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No mobile app</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No mobile app</p>
