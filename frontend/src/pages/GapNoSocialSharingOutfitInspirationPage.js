...
// Gap (Non-AI): No social sharing (outfit inspiration)
// No social sharing (outfit inspiration)
import React, { useState } from 'react';

export default function GapNoSocialSharingOutfitInspirationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-social-sharing-outfit-inspiration/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No social sharing (outfit inspiration)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No social sharing (outfit inspiration)</p>
