...
// Gap (Non-AI): Only 8 frontend pages
// Only 8 frontend pages
import React, { useState } from 'react';

export default function GapOnly8FrontendPagesPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-only-8-frontend-pages/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Only 8 frontend pages</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Only 8 frontend pages</p>
