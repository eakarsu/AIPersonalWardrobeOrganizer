...
// Gap Feature: Wear
// Wear-logs without /wear-pattern-analysis (identify favorite pieces, unused items)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalWardrobeOrganizer / gap-wear',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Wear'.\nProject: AIPersonalWardrobeOrganizer.\nContext: Wear-logs without /wear-pattern-analysis (identify favorite pieces, unused items)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['wear', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'wear', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
