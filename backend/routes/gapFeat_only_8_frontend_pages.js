...
// Gap Feature: Only 8 frontend pages
// Only 8 frontend pages
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalWardrobeOrganizer / gap-only-8-frontend-pages',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Only 8 frontend pages'.\nProject: AIPersonalWardrobeOrganizer.\nContext: Only 8 frontend pages\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['only-8-frontend-pages', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'only-8-frontend-pages', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
