...
// Trip details input (duration, activities, weather); AI generates minimal packing list from existing wardrobe with predicted cost-per-day
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalWardrobeOrganizer / cf-travel-capsule-builder',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_travel_capsule_builder`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-travel-capsule-builder feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Travel capsule builder'.
Context: Trip details input (duration, activities, weather); AI generates minimal packing list from existing wardrobe with predicted cost-per-day
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-travel-capsule-builder/run error:', err);
