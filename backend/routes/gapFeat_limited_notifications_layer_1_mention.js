...
// Gap Feature: Limited notifications layer (1 mention)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalWardrobeOrganizer / gap-limited-notifications-layer-1-mention',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited notifications layer (1 mention)'.\nProject: AIPersonalWardrobeOrganizer.\nContext: Limited notifications layer (1 mention)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-notifications-layer-1-mention', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-notifications-layer-1-mention', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
