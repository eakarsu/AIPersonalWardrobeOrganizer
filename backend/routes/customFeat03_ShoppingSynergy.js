...
// Recommend next purchase that maximizes outfit combinations; calculate cost-per-wear for investment recommendations
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalWardrobeOrganizer / cf-shopping-synergy',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_shopping_synergy`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-shopping-synergy feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Shopping synergy'.
Context: Recommend next purchase that maximizes outfit combinations; calculate cost-per-wear for investment recommendations
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-shopping-synergy/run error:', err);
