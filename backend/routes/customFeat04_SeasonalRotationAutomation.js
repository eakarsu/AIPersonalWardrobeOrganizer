...
// AI predicts seasonal transitions; suggests rotation timing; identifies items that didn\'t work (to remove)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalWardrobeOrganizer / cf-seasonal-rotation-automation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_seasonal_rotation_automation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-seasonal-rotation-automation feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Seasonal rotation automation'.
Context: AI predicts seasonal transitions; suggests rotation timing; identifies items that didn\'t work (to remove)
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-seasonal-rotation-automation/run error:', err);
