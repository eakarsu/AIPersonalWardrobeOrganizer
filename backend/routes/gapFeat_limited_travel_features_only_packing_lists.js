...
// Gap Feature: Limited travel features (only packing lists)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalWardrobeOrganizer / gap-limited-travel-features-only-packing-lists',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited travel features (only packing lists)'.\nProject: AIPersonalWardrobeOrganizer.\nContext: Limited travel features (only packing lists)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-travel-features-only-packing-lists', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-travel-features-only-packing-lists', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
