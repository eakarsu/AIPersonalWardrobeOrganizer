...
// Gap Feature: No size/fit tracking (for new purchase recommendations)
// No size/fit tracking (for new purchase recommendations)
// Project: AIPersonalWardrobeOrganizer
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalWardrobeOrganizer / gap-no-size-fit-tracking-for-new-purchase-recommendati',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No size/fit tracking (for new purchase recommendations)'.\nProject: AIPersonalWardrobeOrganizer.\nContext: No size/fit tracking (for new purchase recommendations)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-size-fit-tracking-for-new-purchase-recommendati', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-size-fit-tracking-for-new-purchase-recommendati', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-size-fit-tracking-for-new-purchase-recommendati']
    );
