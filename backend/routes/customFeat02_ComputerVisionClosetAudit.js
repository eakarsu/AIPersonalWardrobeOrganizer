...
// Batch process photos of closet; AI auto-tags all items, detects duplicates, quality-assesses condition; suggests decluttering
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalWardrobeOrganizer / cf-computer-vision-closet-audit',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_computer_vision_closet_audit`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-computer-vision-closet-audit feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Computer vision closet audit'.
Context: Batch process photos of closet; AI auto-tags all items, detects duplicates, quality-assesses condition; suggests decluttering
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-computer-vision-closet-audit/run error:', err);
