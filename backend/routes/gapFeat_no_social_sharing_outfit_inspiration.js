...
// Gap Feature: No social sharing (outfit inspiration)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalWardrobeOrganizer / gap-no-social-sharing-outfit-inspiration',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No social sharing (outfit inspiration)'.\nProject: AIPersonalWardrobeOrganizer.\nContext: No social sharing (outfit inspiration)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-social-sharing-outfit-inspiration', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-social-sharing-outfit-inspiration', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
