...
// Daily outfit recommendation based on weather, calendar, laundry status, and wear frequency; auto-suggests new purchases to prevent boredom
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPersonalWardrobeOrganizer / cf-outfit-orchestration-agent',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_outfit_orchestration_agent`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-outfit-orchestration-agent feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Outfit orchestration agent'.
Context: Daily outfit recommendation based on weather, calendar, laundry status, and wear frequency; auto-suggests new purchases to prevent boredom
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-outfit-orchestration-agent/run error:', err);
